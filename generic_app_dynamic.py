...
if st.session_state.get("loaded"):
    ...
    with col_a:
        field_path = st.text_input("Field path to find (example)", value=f"{root_field}.sysIdentifiers.mdmId")
    with col_b:
    ...
    st.header("Search field path")
    st.text_input("Field path (load data first)", value=f"{root_field}.sysIdentifiers.mdmId", disabled=True)
